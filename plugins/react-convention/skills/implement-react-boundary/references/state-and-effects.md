...
蔵書の検索画面で、検索語と「貸出可能だけ」のfilterはURLに置く。filterを変えたらpageは一ページ目に戻る。この依存はURLのschemaが持つ。検索語の入力欄の途中の文字は、確定するまでURLに入れない。「延滞している本の数」は、貸出の一覧から描画中に数える。stateに写すと、延長の後に数がずれる。
