...
readのServer Function（GET）では状態を変えない。GETは再試行や先読みで何度も呼ばれうるからである。本を借りる、返す、logoutのように状態を変える処理は、必ずPOSTのServer Functionに置く。
...
Server Functionは、成功と、予測できる失敗の種類を判別できるunionで返す（`{ kind: "borrowed" }`、`{ kind: "rejected", reason: "overdue" }` のように）。失敗の種類は業務の資料にある拒む理由だけにし、画面の側で拒む理由を作らない。表示側は、その種類を網羅して描き分ける。未知の種類を受けたら、成功や既定の表示へ丸めず、想定外の失敗として扱う。想定外の失敗（通信の断、壊れた応答）は結果の型へ詰めずにthrowし、Error Boundaryに受けさせる。未認証のredirectや存在しないresourceの応答は、Error Boundaryへ届く前に入口で返す。
...
同じものを複数の利用者が並行して編集しうるときは、保存の単位ごとに版（version）を持ち、下書きを始めた時点の版を控える。新しいreadの版が控えと違えば他者の変更であり、下書きを黙って上書きせず、取り込むかを利用者に選ばせる。一人しか編集しないものには版を持たせない。使われない仕組みがformごとに増えるだけだからである。二度実行すると結果が変わる操作（送付、課金、外部への通知）は、冪等のkeyを付けて送る。

## 判断例（図書館の貸出）

「本を借りる」は、貸出のServer Function（POST）に置く。関数の内側で、sessionの利用者を得て、その利用者として借りる要求をbackendへ送る。業務の資料が定める拒む理由は二つである。延滞の貸出がある利用者は借りられない（`{ kind: "rejected", reason: "overdue" }`）。貸出上限に達している利用者は借りられない（`{ kind: "rejected", reason: "loan_limit" }`）。表示側は二つの理由を別の文で示す。backendが知らない理由を返してきたら、「借りられませんでした」と丸めずにthrowし、Error Boundaryで扱う。

蔵書の一覧のURLの `?page=abc` は、schemaで数値として検証し、合わなければ一ページ目へ黙って戻さず、誤ったページの指定であることを表示する。`page` が無いときは、仕様どおり一ページ目を表示する。
